#!/usr/bin/env bash
# Quick setup per creare il pacchetto .deb di KB Search
...
log "🚀 Setup per creazione pacchetto .deb KB Search..."

# Check if we're in the right directory
if [[ ! -f "apps/kb-ui/assets/app.js" ]]; then
    echo "❌ Esegui questo script dalla directory root del progetto KB Search"
    exit 1
fi

log "📋 Creazione files necessari per build .deb..."

# 1. Create the main builder script
log "Creazione build-deb-package.sh..."
cat > build-deb-package.sh << 'BUILDER_EOF'
#!/usr/bin/env bash
...
success "✅ build-deb-package.sh creato!"

# 2. Create simple Makefile
log "Creazione Makefile..."
...
success "✅ Makefile creato!"

echo
echo -e "${GREEN}🎉 SETUP COMPLETATO! 🎉${NC}"
echo
echo -e "${BLUE}📋 Prossimi passi:${NC}"
echo "  1. Build pacchetto: ./build-deb-package.sh"
echo "     oppure: make deb"
echo
echo "  2. Install pacchetto: sudo dpkg -i kbsearch_2.0.0_all.deb"
echo "     oppure: make install"
echo
echo -e "${BLUE}📁 File creati:${NC}"
echo "  ✅ build-deb-package.sh (script principale)"
echo "  ✅ Makefile (automazione build)"
echo
success "Tutto pronto per creare il pacchetto .deb!"

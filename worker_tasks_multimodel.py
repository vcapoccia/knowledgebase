# worker/worker_tasks.py - MULTI-MODEL EDITION
import os
import json
import subprocess
import logging
...
def _add_processing_log(rc: Redis, entry: Dict[str, Any]):
    ...
    rc.ltrim(Q_REDIS_KEY_PROCESSING_LOG, 0, 99)  # Mantieni solo ultimi 100
...
def _chunk_text(text: str, chunk_size: int = 1000, overlap: int = 200) -> List[str]:
    ...
    while start < len(text):
        ...
        # Trova ultimo punto per evitare split a metà frase
        if end < len(text):
...
def extract_metadata(filepath: str, kb_root: str = KB_ROOT) -> Dict[str, Optional[str]]:
    ...
    Supporta:
    - _AQ/SD{N}/99_AS/AS{codice}/04_OffertaTecnica/doc.docx
    - _Gare/{ANNO}_{Cliente}-{Oggetto}/01_Documentazione/doc.pdf
    """
    import re
...
def run_ingestion(params: Dict[str, Any]) -> Dict[str, Any]:
    """
    Task principale di ingestion multi-modello.
    ...
    log.info(f"🚀 Avvio ingestion: mode={mode}, model={model_type}, collection={collection_name}")
    ...
    with pg, pg.cursor() as cur:
        for path in all_files:
            ...
            try:
                ...
                # 5. Embeddings
                _set_current_doc(rc, {"filename": filename, "path": path, "step": "embedding", "details": f"{len(chunks)} chunks"})
                ...
                meili_batch.append({
                    ...
                    "path": path,
                    "title": title,
                    "content": text[:5000],  # Limita per Meilisearch
                    **{k: v for k, v in metadata.items() if v is not None}
                })
                ...
                _update_stats(rc,
                    success=done + 1,
                    chunked=sum([len(_chunk_text(_read_text(f))) for f in all_files[:done+1] if _read_text(f)]),
    ...
    log.info(f"✅ Ingestion completata: {total} documenti, modello {model_type}, collection {collection_name}")
